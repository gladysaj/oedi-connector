...
export async function retractConnection(
  ...
  if (!connectionReady) {
    context.warn("Connection not ready yet...");
    return { status: 503, body: "Connection not ready" };
  }
...
export async function clearConnection(
  ...
  if (fullCrawlInProgress) {
    ...
    return { status: 409, body: "Full crawl in progress" };
  }
...
app.timer("incrementalCrawl", {
  // Runs every 6 hours (OpenEI has no delta filter, so this is effectively a full crawl)
  schedule: "0 */6 * * *",
  runOnStartup: false,
