...
export async function clearConnectionItems(config: Config): Promise<boolean> {
  try {
    ...
    if (connection) {
      ...
      for await (const item of getAllItems(config, undefined)) {
        try {
          await client.api(`/external/connections/${config.connector.id}/items/${item.id}`).delete();
          config.context.log(`Deleted item ${item.id}`);
        } catch (e) {
          config.context.error(`Failed to delete item ${item.id}: ${e.message}`);
        }
      }
      return true;
    }
